...
import '../css/ChatsPage.css'; // We'll create this CSS file next
...
const ChatsPage = () => {
  ...
  // Load user chats and handle incoming state
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`http://localhost:5000/api/chats/user/${user.user_id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        setChats(response.data);
        
        // Check for incoming chat data from navigation
        if (location.state?.chatData) {
          ...
          if (existingChat) {
            ...
            const newChatResponse = await axios.post('http://localhost:5000/api/chats', 
      ...
      const fetchMessages = async () => {
        try {
          ...
          const response = await axios.get(
            `http://localhost:5000/api/chats/${activeChat.chat_id}/messages`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
